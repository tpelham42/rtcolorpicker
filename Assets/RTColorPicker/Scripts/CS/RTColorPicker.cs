...
namespace RTColorPickerCS{
	...
	public class RTColorPicker : MonoBehaviour {
		...
		//Texture used to sample screen images for color picker
		...
		public IEnumerator UpdateZoomedTexture() {
			if (ZoomedTex == null) {
				Debug.Log(this + " :: Error Updating Zoomed Texture. ZoomedText Texture2D property is null!");
				yield return null;
			}

			while (_eyeDropperActive) {
		...
		int DrawButtonRow(Rect groupRect){
			...
			if(AllowEyeDropper){
				if(GUI.Button(new Rect(groupRect.width-80, 2, 15, 15), new GUIContent("", "Sample Color"), EyeDropperBtnStyle)){
					...
					StartCoroutine("UpdateZoomedTexture");
				}//if
			}//if				
		...
		public void LoadCustomColors(string dataStr){	
			...
			OnCustomColorLoadedEvent?.Invoke();
